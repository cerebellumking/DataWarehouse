        ...
        <el-form ref="form" :model="form" label-width="120px" style="padding-top: 10vh;">
          ...
          <el-form-item label="主演">
            ...
            <el-autocomplete
              ...
              class="input-new-tag"
              size="small"
              :fetch-suggestions="mainActorSearchSuggest"
              placeholder="请输入内容"
              style="width: 20vw;"
              clearable
        ...
        <el-tabs v-model="activeName" @tab-click="handleClick">
          <el-tab-pane label="查询结果" name="first">
            ...
            <el-table v-loading="movieLoading" height="460" border stripe :data="movieData" style="width: 100%">
                ...
                <template slot-scope="props">
                  <el-form label-position="left" inline class="demo-table-expand">
                    ...
                    <el-form-item label="版本数">
                      <span>{{ props.row.formatNum }}</span>
                    </el-form-item>
                    <el-form-item label="上映时间">
                      <span>{{ props.row.movieTime }}</span>
                    </el-form-item>
                    <el-form-item  v-if="props.row.movieStyle.length!==0" label="风格">
                      <span v-for="i in props.row.movieStyle">{{ i }},</span>
                    </el-form-item>
                    ...
                    <el-form-item label="评论数量">
                      <span>{{ props.row.reviewNum }}</span>
                    </el-form-item>
                    <el-form-item label="正面评论比例">
              ...
              <el-table-column prop="movieName" label="名称" width="250" />
  ...
  methods: {
    ...
    searchMovie(){
      ...
      if (this.form.movieName!=""){
        searchText+=" 电影名为"+this.form.movieName+""
        searchCondition.movieName=this.form.movieName
      }
      console.log(this.form)
      if(this.form.movieDate!=null && this.form.movieDate.length!=0){
        console.log(this.form.movieDate)
      ...
      if (this.form.style != ""){
        searchText+=" 类别为"+this.form.style+""
        searchCondition.style=this.form.style
      }
      ...
      if(this.form.positive!=0){
        searchCondition.positive = this.form.positive/100
        searchText+=" 正面评价在"+ this.form.positive +"%之上"
      }
      // 设置参数
      ...
      searchCondition.pageNo = this.pageNo;
      searchCondition.pageSize = this.pageSize;
      searchText+=" 的电影"
      this.searchText=searchText
      this.vchartsConfig.extend.title.subtext=searchText//TOSEE

      this.relationReady=false//TOSEE
      this.distributeReady=false//TOSEE
      this.graphReady=false//TOSEE
      ...
      axios({
        method: 'post',
        url: this.BASE_URL+'/mysql/movie/association/result',
      ...
      axios({
        method:'post',
        url: this.HIVE_BASE_URL+'/hive/movie/association/result',
        data:searchCondition,
        headers: { }
      }).then(response=>{
        console.log("hive")
      ...
      var config = {
        method: 'post',
        url: this.NEO4J_BASE_URL+'/neo4j/movie/association/result',
        data:searchCondition ,
        headers: { 
        }
      };
      ...
      .then(response=> {
        ...
        this.movieNumber = response.data.totalMovieNum
        
        for(let i=0;i<this.movieData.length;++i){
          this.movieData[i].score = this.movieData[i].score.toFixed(2)
          this.movieData[i].positive = this.movieData[i].positive * 100
        }
    ...
    exampleTest(){
      ...
      // this.form.maxDay = 15;
      this.form.style='Adventure';
      let startTime = new Date(2000,12,5); 
      let endTime = new Date(2014,1,10); 
      this.form.movieDate=[startTime,endTime];
      this.form.movieDirectors = ['MichelHazanavicius']
      this.form.movieMainActors = ['JeanDujardin']
      this.form.positive = 48
      this.searchMovie();
    },
  }
}
